...
class PolskiLadContractOfEmployment {
  /**
   * Kwota netto
   * @type {number}
   */
  ...
  calculateExpenses () {
    ...
    if (expenses > constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD) {
      this.expenses = constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD
    }
  ...
  calculateUSEmployeeHealth () {
    ...
    if (this.gross <= constants.CONTRACT_OF_EMPLOYMENT.LUMP_SUM_UP_TO_AMOUNT) {
      USHealthEmployee = (constants.ZUS.EMPLOYEE.POLSKI_LAD_HEALTH_RATE / 100) *
        (this.gross - (this.employeeZus.pension +
          this.employeeZus.rent + this.employeeZus.sick))
    } else {
      USHealthEmployee = (constants.US.EMPLOYEE.POLSKI_LAD_HEALTH_RATE / 100) *
        (this.gross - (this.employeeZus.pension +
          this.employeeZus.rent + this.employeeZus.sick))
    }
  ...
  calculateAll (young, fp, ppk) {
    ...
    if (this.basicAmountForRentAndPension > constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD) {
      this.basicAmountForRentAndPension = constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD
    }
...
export default PolskiLadContractOfEmployment
