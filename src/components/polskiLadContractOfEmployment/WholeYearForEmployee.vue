...
export default {
  ...
  created () {
    ...
    if (this.totalBasisForTax > this.$constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD) {
      this.$q.notify({
  ...
  computed: {
    ...mapGetters({
      ...
      authorExpensePart: 'polskiLadContractOfEmployment/authorExpensePart',
      ...
      freeAmount: 'polskiLadContractOfEmployment/freeAmount',
    }),
  },
  methods: {
    setData () {
    ...
    getResultForOneMonth (month) {
      ...
      if (currentTotalExpenses > this.$constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD) {
        model.expenses = 0
      } else {
        if (newTotalExpenses > this.$constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD) {
          model.expenses = this.$constants.AMOUNT_OF_POLSKI_LAD_TAX_THRESHOLD - currentTotalExpenses
        }
      }
