...
const labels = ['Oddzielnie', 'Wspólnie']

const chartData = computed(() => {
  const taxAmount =
    store.husbandResult && store.wifeResult
      ? store.husbandResult.taxAmount + store.wifeResult.taxAmount
      : 0
  return useBarChart('Podatek dochodowy', labels, [
    taxAmount,
