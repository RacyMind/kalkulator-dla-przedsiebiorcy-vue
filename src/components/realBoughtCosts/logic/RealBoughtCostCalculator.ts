...
export class RealBoughtCostCalculator extends BasicCalculator<InputFields, Result> implements Calculator<InputFields, Result>{
  ...
  public calculate(): this {
    ...
    if(Object.values(incomeTaxConstants.value.taxScale.taxRates).includes(this.getInputData().incomeTaxRate)) {
      healthContribution = zus.getHealthContribution(priceWithoutVatTax, EntrepreneurTaxSystem.TaxScale, 0, 0, true)
    } else if(this.getInputData().incomeTaxRate === incomeTaxConstants.value.flatTax.taxRate) {
      healthContribution = zus.getHealthContribution(priceWithoutVatTax, EntrepreneurTaxSystem.FlatTax, 0, 0, true)
    }

    const savedAmount = helpers.round(deductedVatTaxAmount + deductedIncomeTaxAmount + healthContribution, 2)
