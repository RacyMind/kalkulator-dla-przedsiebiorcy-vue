<template>
  ...
  >
    ...
    <FormSection title="Typ limitu IKZE">
      <div class="row items-center">
        ...
        <div class="col-auto q-ml-sm">
          <Tooltip>
            <p>
              <strong>Umowa o pracę</strong> — niższy limit IKZE (1,2-krotność
              przeciętnego wynagrodzenia).
            </p>
            <p>
              <strong>Działalność gospodarcza</strong> — wyższy limit IKZE
      ...
      <div class="row q-mt-sm">
        <div class="col">
          Limit IKZE na {{ settingStore.dateOfLawRules.getFullYear() }} rok:
          <strong>{{ pln(currentIkzeLimit) }}</strong>
        </div>
      </div>
    </FormSection>
    <FormSection
    ...
    >
      ...
      <div v-if="taxSystem === IkzeTaxSystem.LumpSum" class="row q-mt-sm">
        <div class="col">
          <q-select
        ...
        <div class="col">
          <q-input
            ...
            :rules="[
              validationRules.requiredAmount,
              validationRules.minValue(0),
              validationRules.maxValue(currentIkzeLimit),
            ]"
            lazy-rules="ondemand"
            hide-bottom-space
            aria-required="true"
...
const handleFormSubmit = () => {
  const inputFields: InputFields = {
    status: status.value,
    taxSystem:
      status.value === IkzeLimitStatus.EmploymentContract
        ? IkzeTaxSystem.TaxScale
        : taxSystem.value,
