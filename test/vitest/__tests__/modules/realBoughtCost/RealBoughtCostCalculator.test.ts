...
describe('Calculator for real bought costs on 1.01.2024', () => {
  ...
  it('The invalid data', () => {
    expect(() => new RealBoughtCostCalculator().getResult()).toThrowError(
      'undefined',
    )
    expect(() =>
      new RealBoughtCostCalculator().calculate().getResult(),
    ).toThrowError('undefined')
  })

  it('with the deducted vat and income taxes', () => {
    const result = new RealBoughtCostCalculator()
      .setInputData(getDefaultInput())
      .calculate()
      .getResult()
    ...
    expect(result.realCost).toBe(79)
  })
  ...
  it('with the deducted income tax', () => {
    ...
    expect(result.realCost).toBe(82)
  })
  ...
  it('with the deducted income flat tax', () => {
    ...
    expect(result.realCost).toBe(99.1)
  })

  it('with no deductions', () => {
    const result = new RealBoughtCostCalculator()
      .setInputData({
        ...getDefaultInput(),
        deductedVatTaxPart: 0,
    ...
    expect(result.realCost).toBe(123)
  })
})
