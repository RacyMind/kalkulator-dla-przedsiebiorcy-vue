...
describe('Annual Employer Calculator of Contract of Employment on 1.11.2023', () => {
  ...
  it('The invalid data', () => {
    expect(() => new AnnualEmployerCalculator().getResult()).toThrowError(
      'undefined',
    )
    expect(() =>
      new AnnualEmployerCalculator().calculate().getResult(),
  ...
  describe('Test ZUS contributions', () => {
    ...
    it('The accident contribution', () => {
      expect(
        new AnnualEmployerCalculator()
          .setInputData(annualInput(input))
          .calculate()
          .getResult().annualResult.accidentContribution,
      ).toBe(200.4)
    ...
    it('The FP (+ all related)  contributions', () => {
      const result = new AnnualEmployerCalculator()
        .setInputData(annualInput(input))
        .calculate()
      ...
      const resultWithoutContributions = new AnnualEmployerCalculator()
        .setInputData(
          annualInput({
            ...input,
  ...
  describe('Test the gross and total amount', () => {
    ...
    it('Higher gross amount increases annual employer result values', () => {
      const baseResult = new AnnualEmployerCalculator()
        .setInputData(
          annualInput({
            ...input,
            grossAmount: 4000,
      ...
      const higherGrossResult = new AnnualEmployerCalculator()
        .setInputData(
          annualInput({
            ...input,
            grossAmount: 5000,
      ...
      expect(higherGrossResult.totalAmount).toBeGreaterThan(
        baseResult.totalAmount,
      )
      ...
      expect(higherGrossResult.fpContribution).toBeGreaterThan(
        baseResult.fpContribution,
      )
      expect(higherGrossResult.fgspContribution).toBeGreaterThan(
        baseResult.fgspContribution,
      )
      expect(higherGrossResult.fsContribution).toBeGreaterThan(
        baseResult.fsContribution,
      )
      expect(higherGrossResult.accidentContribution).toBeGreaterThan(
        baseResult.accidentContribution,
      )
    })
  })
})
